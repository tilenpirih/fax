...
// gcc seminarska.c -Wl,-rpath,./ -L./ -l:"libfreeimage.so.3" -lm -o seminarska
// Uporabi float!
// ker je 32 bitna slika uporabi pitch
...
int main(){
    ...
    /*
    tabelaGaussRazdalje1 = (double *)malloc(pow(w + 1, 2) * sizeof(double));
    for(int i=0; i<pow(w + 1, 2); i++){
        int x = i / (w + 1);
        int y = i % (w + 1);
        //printf("%d %d\n", x, y);
        tabelaGaussRazdalje1[i] = exp(-((pow(x, 2) + pow(y, 2)) / (2 * pow(SIGMA_S, 2))));
    }*/
